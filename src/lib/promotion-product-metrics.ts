...
const MIN_LIST_VIEW_SESSIONS = 20;
...
scoped_events AS (
  SELECT e.*
  FROM events e
  WHERE e.event_name IN ('view_item_list', 'select_item', 'add_to_cart', 'add_to_wishlist')
...
scoped_events AS (
  ...
  WHERE e.event_timestamp >= l.first_landing.event_timestamp
    AND e.event_name IN ('view_item_list', 'select_item', 'add_to_cart', 'add_to_wishlist')
...
WITH base AS (
  ...
  WHERE _TABLE_SUFFIX BETWEEN @suffixFrom AND @suffixTo
    AND event_name IN ('page_view', 'view_item_list', 'select_item', 'add_to_cart', 'add_to_wishlist')
...
export async function readPromotionProductMetrics(input: {
  ...
  const rows = queryRows.map((row): PromotionProductMetricRow => {
    ...
    if (!product) unmatchedGoodsRefs += 1;
    ...
    return {
      ...
      name: product?.name || `goods_ref ${goodsRef}`,
  ...
  const listToProduct = topRows(
    stockRows,
    (row) => row.listImpressions >= MIN_LIST_VIEW_SESSIONS,
