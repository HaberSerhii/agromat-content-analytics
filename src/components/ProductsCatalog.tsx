...
const loadProductCatalogSecondaryViews = () => import("@/components/ProductCatalogSecondaryViews")
  .then((module) => module.ProductCatalogSecondaryViews);

const ProductCatalogSecondaryViews = dynamic(loadProductCatalogSecondaryViews, {
  ssr: false,
  loading: () => (
    <Card>
      <div className="py-16 text-center text-xs font-semibold" style={{ color: "var(--text-dim)" }}>
...
function preloadCatalogMode(mode: CatalogMode) {
  if (mode !== "catalog") void loadProductCatalogSecondaryViews();
}
...
function RequiredAttrsModal({ onClose }: {
  onClose: () => void;
}) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/products/filters?view=categories").then((r) => r.json()),
    ])
      .then(([cfg, data, filterData]: [
        Record<string, number[]>,
        { categories: CategoryAggregate[]; excluded: number[] },
        { categories: FilterCategory[] },
      ]) => {
        if (cancelled) return;
        setConfig(cfg || {});
...
// Secondary analytics, timeline and competitor-price modes load on demand.
...
export function ProductsCatalog() {
  ...
  const loadList = useCallback(() => {
    if (skipEffectiveStatsReload.current) {
      skipEffectiveStatsReload.current = false;
      return;
    }
    ...
    fetchProductsQuery(buildQuery())
      ...
      .then((d: ListResponse) => {
        setData(d);
        if (!catalogStatsFrom && !catalogStatsTo && d.statsFrom && d.statsTo) {
          skipEffectiveStatsReload.current = true;
          setCatalogStatsFrom(d.statsFrom);
          setCatalogStatsTo(d.statsTo);
        }
      })
      .catch((e) => setError(e instanceof Error ? e.message : "Error"))
      .finally(() => setLoading(false));
  }, [buildQuery, catalogStatsFrom, catalogStatsTo]);
  ...
  return (
    <>
            ...
            <div className="flex gap-1 rounded-xl p-0.5" style={{ background: "var(--bg-input)", border: "1px solid var(--border2)" }}>
              {([
                ...
                <button
                  key={m}
                  onClick={() => setMode(m)}
                  onMouseEnter={() => preloadCatalogMode(m)}
                  onFocus={() => preloadCatalogMode(m)}
      ...
      {openReq && filters && <RequiredAttrsModal onClose={() => { setOpenReq(false); loadList(); }} />}
      {openSettings && data && (
        <SettingsModal
