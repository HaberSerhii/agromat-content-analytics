...
TMP_CRON="$(mktemp)"
crontab -l 2>/dev/null | grep -v "run-products-sync.sh" | grep -v "run-agromat-price-sync.sh" | grep -v "run-simple-price-auto.sh" | grep -v "prewarm-dashboard-cache.sh" | grep -v "run-content-review-check.sh" > "$TMP_CRON" || true
