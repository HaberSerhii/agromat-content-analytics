...
TMP_CRON="$(mktemp)"
crontab -l 2>/dev/null | grep -v "run-products-sync.sh" | grep -v "run-agromat-price-sync.sh" | grep -v "run-simple-price-auto.sh" > "$TMP_CRON" || true
...
echo "Price cron:   morning parser chain (after Rozetka)"
