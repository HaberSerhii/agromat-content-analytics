...
PRICE_POSITION_SNAPSHOT_DIR_VALUE="${PRICE_POSITION_SNAPSHOT_DIR:-$(dirname "$SNAPSHOT_DIR")/price-position-snapshots}"
...
CONTENT_REVIEW_LOG="${CONTENT_REVIEW_LOG:-/var/log/agromat-content-review.log}"
...
run_root mkdir -p "$SNAPSHOT_DIR" "$PROMOTIONS_SNAPSHOT_DIR_VALUE" "$PRICE_POSITION_SNAPSHOT_DIR_VALUE" "$DASHBOARD_CACHE_DIR_VALUE"
...
run_root touch "$CONTENT_REVIEW_LOG"
run_root chown "$APP_USER":"$APP_USER" "$CONTENT_REVIEW_LOG"
run_root touch "$PRICE_POSITION_SNAPSHOT_LOG"
run_root chown "$APP_USER":"$APP_USER" "$PRICE_POSITION_SNAPSHOT_LOG"
...
set_env "PRICE_POSITION_SNAPSHOT_DIR" "$PRICE_POSITION_SNAPSHOT_DIR_VALUE" "$APP_DIR/.env"
...
CONTENT_REVIEW_CRON_LINE="25 10 * * * APP_DIR=$APP_DIR APP_PORT=$APP_PORT CONTENT_REVIEW_LOG=$CONTENT_REVIEW_LOG $APP_DIR/scripts/run-content-review-check.sh"
...
TMP_CRON="$(mktemp)"
crontab -l 2>/dev/null | grep -v "run-products-sync.sh" | grep -v "run-agromat-price-sync.sh" | grep -v "run-simple-price-auto.sh" | grep -v "prewarm-dashboard-cache.sh" | grep -v "run-content-review-check.sh" | grep -v "run-price-position-snapshot.sh" > "$TMP_CRON" || true
...
echo "Price-position snapshots: $PRICE_POSITION_SNAPSHOT_DIR_VALUE"
...
echo "Content review log:  $CONTENT_REVIEW_LOG"
echo "Content review cron: $CONTENT_REVIEW_CRON_LINE"
echo "Price-position snapshot log:  $PRICE_POSITION_SNAPSHOT_LOG"
